\subsection{Model-dependent isobar decomposition}
    ...
    $\mathcal{F}$ is the Blatt-Wei\ss{}kopf form factor (see the table~\ref{table:blatt_weisskopf}), which depends on $R$, the resonance \emph{radial size}\index{radial size};
    ...
    The \Pfnez{} resonance lies close to both the \Ppiplus{}\Ppiminus{} and the \PKplus{}\PKminus{} decay channels and is usually parametrized by a Flatté\index{Flatte@Flatté} distribution, which takes into account the opening of the second channel:
    ...
    and $\rho_{1\!}(s)$ and $\rho_2(s)$ the (complex) phase-space factors~\cite{FLATTE1976224}.\marginpar{which are the p-s factors?}
